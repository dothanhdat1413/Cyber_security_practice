// aes.c - Full AES-128 Implementation

#include "aes.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define Nb 4
#define Nk 4
#define Nr 10

static uint8_t sbox[256] = {
    
};

static uint8_t rsbox[256] = {
    
};

static uint8_t Rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
};

static uint8_t xtime(uint8_t x) {
    return (x << 1) ^ ((x >> 7) * 0x1b);
}

static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
    int i, j;
    uint8_t temp[4];
    for (i = 0; i < Nk; ++i) {
        for (j = 0; j < 4; ++j)
            RoundKey[i * 4 + j] = Key[i * 4 + j];
    }
    for (i = Nk; i < Nb * (Nr + 1); ++i) {
        for (j = 0; j < 4; ++j)
            temp[j] = RoundKey[(i - 1) * 4 + j];
        if (i % Nk == 0) {
            uint8_t k = temp[0];
            temp[0] = sbox[temp[1]] ^ Rcon[i / Nk];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[k];
        }
        for (j = 0; j < 4; ++j)
            RoundKey[i * 4 + j] = RoundKey[(i - Nk) * 4 + j] ^ temp[j];
    }
}

static void AddRoundKey(uint8_t* state, const uint8_t* RoundKey, int round) {
    for (int i = 0; i < 16; ++i)
        state[i] ^= RoundKey[round * 16 + i];
}

static void SubBytes(uint8_t* state) {
    for (int i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];
}

static void InvSubBytes(uint8_t* state) {
    for (int i = 0; i < 16; ++i)
        state[i] = rsbox[state[i]];
}

static void ShiftRows(uint8_t* state) {
    uint8_t temp[16];
    memcpy(temp, state, 16);

    state[0]  = temp[0];  state[1]  = temp[5];  state[2]  = temp[10]; state[3]  = temp[15];
    state[4]  = temp[4];  state[5]  = temp[9];  state[6]  = temp[14]; state[7]  = temp[3];
    state[8]  = temp[8];  state[9]  = temp[13]; state[10] = temp[2];  state[11] = temp[7];
    state[12] = temp[12]; state[13] = temp[1];  state[14] = temp[6];  state[15] = temp[11];
}

static void InvShiftRows(uint8_t* state) {
    uint8_t temp[16];
    memcpy(temp, state, 16);

    state[0]  = temp[0];  state[1]  = temp[13]; state[2]  = temp[10]; state[3]  = temp[7];
    state[4]  = temp[4];  state[5]  = temp[1];  state[6]  = temp[14]; state[7]  = temp[11];
    state[8]  = temp[8];  state[9]  = temp[5];  state[10] = temp[2];  state[11] = temp[15];
    state[12] = temp[12]; state[13] = temp[9];  state[14] = temp[6];  state[15] = temp[3];
}

static void MixColumns(uint8_t* state) {
    uint8_t tmp[16];
    for (int i = 0; i < 4; ++i) {
        int j = i * 4;
        tmp[j]   = xtime(state[j]) ^ xtime(state[j+1]) ^ state[j+1] ^ state[j+2] ^ state[j+3];
        tmp[j+1] = state[j] ^ xtime(state[j+1]) ^ xtime(state[j+2]) ^ state[j+2] ^ state[j+3];
        tmp[j+2] = state[j] ^ state[j+1] ^ xtime(state[j+2]) ^ xtime(state[j+3]) ^ state[j+3];
        tmp[j+3] = xtime(state[j]) ^ state[j] ^ state[j+1] ^ state[j+2] ^ xtime(state[j+3]);
    }
    memcpy(state, tmp, 16);
}

static void Cipher(uint8_t* state, const uint8_t* RoundKey) {
    AddRoundKey(state, RoundKey, 0);
    for (int round = 1; round < Nr; ++round) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, RoundKey, round);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, RoundKey, Nr);
}

// Helper function for InvMixColumns
static uint8_t mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    uint8_t high_bit;
    for (int i = 0; i < 8; i++) {
        if (b & 1) 
            p ^= a;
        high_bit = a & 0x80;
        a <<= 1;
        if (high_bit) 
            a ^= 0x1b; // x^8 + x^4 + x^3 + x + 1
        b >>= 1;
    }
    return p;
}

static void InvMixColumns(uint8_t* state) {
    uint8_t tmp[16];
    for (int i = 0; i < 4; ++i) {
        int j = i * 4;
        tmp[j]   = mul(0x0e, state[j]) ^ mul(0x0b, state[j+1]) ^ mul(0x0d, state[j+2]) ^ mul(0x09, state[j+3]);
        tmp[j+1] = mul(0x09, state[j]) ^ mul(0x0e, state[j+1]) ^ mul(0x0b, state[j+2]) ^ mul(0x0d, state[j+3]);
        tmp[j+2] = mul(0x0d, state[j]) ^ mul(0x09, state[j+1]) ^ mul(0x0e, state[j+2]) ^ mul(0x0b, state[j+3]);
        tmp[j+3] = mul(0x0b, state[j]) ^ mul(0x0d, state[j+1]) ^ mul(0x09, state[j+2]) ^ mul(0x0e, state[j+3]);
    }
    memcpy(state, tmp, 16);
}

static void InvCipher(uint8_t* state, const uint8_t* RoundKey) {
    AddRoundKey(state, RoundKey, Nr);
    for (int round = Nr - 1; round > 0; --round) {
        InvShiftRows(state);
        InvSubBytes(state);
        AddRoundKey(state, RoundKey, round);
        InvMixColumns(state);
    }
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(state, RoundKey, 0);
}

void aes_encrypt_file(const char *infile, const char *outfile, const uint8_t key[16]) {
    FILE *fin = fopen(infile, "rb");
    if (!fin) {
        perror("Error opening input file");
        return;
    }
    
    FILE *fout = fopen(outfile, "wb");
    if (!fout) {
        perror("Error opening output file");
        fclose(fin);
        return;
    }
    
    uint8_t block[16];
    uint8_t roundkey[176];
    KeyExpansion(roundkey, key);

    while (1) {
        size_t read = fread(block, 1, 16, fin);
        if (read == 0) break;
        
        // Apply PKCS#7 padding
        uint8_t pad = 16 - read;
        if (read < 16) memset(block + read, pad, pad);
        
        // Encrypt the block
        Cipher(block, roundkey);
        
        // Write encrypted block to output file
        if (fwrite(block, 1, 16, fout) != 16) {
            perror("Error writing to output file");
            fclose(fin);
            fclose(fout);
            return;
        }
    }

    fclose(fin);
    fclose(fout);
    
    printf("File encrypted successfully: %s -> %s\n", infile, outfile);
}

void aes_decrypt_file(const char *infile, const char *outfile, const uint8_t key[16]) {
    FILE *fin = fopen(infile, "rb");
    FILE *fout = fopen(outfile, "wb");
    uint8_t block[16];
    uint8_t roundkey[176];
    KeyExpansion(roundkey, key);

    size_t prev_read = fread(block, 1, 16, fin);
    while (1) {
        uint8_t next_block[16];
        size_t next_read = fread(next_block, 1, 16, fin);

        InvCipher(block, roundkey);

        if (next_read == 0) {
            uint8_t pad = block[15];
            if (pad > 0 && pad <= 16)
                fwrite(block, 1, 16 - pad, fout);
            else
                fwrite(block, 1, 16, fout);
            break;
        } else {
            fwrite(block, 1, 16, fout);
            memcpy(block, next_block, 16);
        }
    }

    fclose(fin);
    fclose(fout);
}
